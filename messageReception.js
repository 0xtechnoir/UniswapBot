...
const provider = new ethers.providers.JsonRpcProvider(mainNetURL + PROJECT_ID);
...
export async function handleMessageReception(message, _token) {
  ...
  if (dexLink) {
    DiscordRequest(`/channels/982706411941285921/messages`, authToken, {
      method: "POST",
      body: {
        content: `@everyone This is not a drill! NEW LINK : ${dexLink[1]}`,
      },
    })
    .then(() => {
...
async function buy(url) {
  ...
  try{
    ...
    if (txGasPrice < gasPriceLimit) {
      ...
      const options = { gasPrice: currentGasPrice, gasLimit: 500000, value: amountIn };    // change back once confirmed its working
      ...
      await DiscordRequest(`/channels/982706411941285921/messages`, authToken, {
        method: "POST",
        body: {
          content: `Buy transction submitted with hash : ${tx.hash}`,
        },
      }); 
...
async function watchPrice() {
  ...
  while (true) {
    ...
    console.log(`Stop loss set at: ${(previousEthAmountOut.div(100).mul(stoploss))*10e-19} ETH (${stoploss}% of previousEthAmountOut)`)
    ...
    console.log(`Current quote: ${ethAmountOut*10e-19} ETH`)
    console.log(`Previous quote: ${previousEthAmountOut*10e-19} ETH`)
...
async function sell(amountOutMin, sellTokenIn, sellTokenOut) {
  ...
  try{
    ...
    if (txGasPrice < gasPriceLimit) {
      ...
      const options = { gasPrice: currentGasPrice, gasLimit: 500000 }; // change back once we know its working   
      ...
      await DiscordRequest(`/channels/982706411941285921/messages`, authToken, {
        method: "POST",
        body: {
          content: `Sell transction submitted with hash : ${tx.hash}`,
        },
      }); 
      ...
      if(receipt) {
        ...
        await DiscordRequest(`/channels/982706411941285921/messages`, authToken, {
          method: "POST",
          body: {
            content: `You closed your trade with ${ethRecieved*10e-19} ETH`,
          },
        }); 
